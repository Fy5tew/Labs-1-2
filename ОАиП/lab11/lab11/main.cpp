...
void downwardTraversal(Tree* t);  //Нисходящий обход дерева
void mixedTraversal(Tree* t);     //Смешанный обход
int calculateLeftPoints(Tree* t); //Подсчет количества левых дочерних вершин
...
void main()
{
	...
	for (;;)
	{
		...
		cout << "9 - Смешанный обход дерева\n";
		cout << "10 - Подсчет количества левых дочерних вершин\n";
		cout << "ваш выбор?\n";
		cin >> choice;
		cout << "\n";
		switch (choice)
		...
		case 10: {
			if (!Root) {
				...
				cout << "Количество левых дочерних вершин: " << calculateLeftPoints(Root) << "\n";
			}
			cout << "\n\n";
			break;
		}
...
void downwardTraversal(Tree* t) {
	if (!t) {
		return;
	}
	cout << t->key << "(" << t->text << ")" << " ";
	downwardTraversal(t->Left);
	downwardTraversal(t->Right);
}

void mixedTraversal(Tree* t) {
	if (!t) {
		return;
	}
	mixedTraversal(t->Left);
	cout << t->key << "(" << t->text << ")" << " ";
	mixedTraversal(t->Right);
}

int calculateLeftPoints(Tree* t) {
	if (!t) {
		return 0;
	}
	return (t->Left ? 1 : 0) + calculateLeftPoints(t->Left) + calculateLeftPoints(t->Right);
}
